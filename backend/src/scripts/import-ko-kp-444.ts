/**
 * Import-Script für Fahrzeug KO-KP-444
 */

import { importSharePointData } from './import-sharepoint';

// SharePoint-Daten für KO-KP-444
const sharePointDaten: Record<string, string | number> = {
  'KO-KP-444': 'KO-KP-444',
  'Titel': 'KO-KP-444',
  'Autohaus': 'Autohaus Kämpflein',
  'Hersteller': 'VW',
  'Modell': 'Golf',
  'Motorisierung': 'Diesel',
  'Einsatzort': 'Bad Marienberg',
  'Nutzer': 'Bote',
  'Kauf/Leasing': 'Kauf',
  'Datum Anschaffung': '10/1/2020',
  'Vereinb. KM': 46000,
  'Kilometerstand': 203648,
  'Meldung KM STand': '1/5/2026',
  'Laufzeit': 48,
  'Laufzeitende': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'Bekannte Probleme': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'Bild': '8b0d189e-8cda-4254-8e67-8d0b68f58ec3',
  'Erinnerung Neuanschaffung': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'Anzahl Werkstatttermine': 0,
  'Erinnerung TÜV': '9/1/2027',
  'Dokumente': 'https://kimmelzahntechnik.sharepoint.com/sites/Fuhrpark-KFZChecklisten/Shared%20Documents/KFZ%20Checklisten/Fuhrparkverwaltung/KO-KP-444',
  'Letzter Werkstatttermin': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'KM-Stand bei Kauf': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'Kostenträger': '19013',
  'Status': 'OK',
  'Status-Datum': '9/1/2025',
  'Waschkarte': '10064-1565',
  'Inkl. Wartung': 'Nein',
  'Profil Winter': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'Profil Sommer': '', // "Geben Sie hier einen Wert ein" - leer lassen
  'KM bis Insp.': '' // "—" - leer lassen
};

try {
  console.log('🚗 Importiere Fahrzeug KO-KP-444...\n');
  const id = importSharePointData(sharePointDaten);
  console.log(`\n✅ Import erfolgreich! Fahrzeug-ID: ${id}`);
} catch (error) {
  console.error('❌ Fehler beim Import:', error);
  process.exit(1);
}
